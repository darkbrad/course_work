...
class UserCRUD:
    def create(self, conn: sqlite3.Connection, data: RegistrationModel) -> None:
        ...
        try:
            ...
            cur.execute(
                ...
                (str(user_id), data.login, passwords.hash_password(data.password),None,1,0),
            )
        finally:
            cur.close()
    ...
    def get(self, conn: sqlite3.Connection, login: str) -> UserModel or None:
        ...
        try:
            cur.execute(
                "SELECT User.id, User.login, User.image,User.bills,User.balance,User.password FROM User WHERE User.login=?",
                (login,),
            )
            row = cur.fetchone()
            ...
            id, login, image, bills,balance,password = row

            if id is None:
                return None

            return UserModel(id=id, login=login, image=image,bills=bills,balance=balance,password=password)
        finally:
            cur.close()
...
SET
    bills = REPLACE(bills,?,?)
WHERE User.login=?''',(int(amount_of_bills),int(amount_of_bills)+1,login),)
        finally:cur.close()
    ...
    def restore_balance(self,conn:sqlite3.Connection,id:str):
        ...
        try:
            ...
            cur.execute("INSERT INTO User VALUES (?,?,?,?,?,?)",(id,login,password,img,bills,start_balance))
        finally:cur.close()
